...
export async function getPackageInfo(params: GetPackageInfoParams): Promise<PackageInfoResponse> {
  ...
  try {
    ...
    const packageInfo = await artifactHubClient.getPackageInfo(package_name);
    
    // Build maintainers list
    const maintainers: AuthorInfo[] = packageInfo.maintainers?.map(m => ({
      name: m.name,
      email: m.email,
    })) || [];
    ...
    if (include_dependencies || include_dev_dependencies) {
      try {
        ...
        const valuesContent = await artifactHubClient.getPackageValues(repo, chart);
          ...
          if (match && include_dependencies) {
            ...
            for (const line of depLines) {
              ...
              if (nameMatch) {
                ...
                currentDep.version = versionMatch[1].trim();
              }
            }
        ...
        logger.debug(`Failed to parse dependencies for ${package_name}`, { error });
      }
    }
